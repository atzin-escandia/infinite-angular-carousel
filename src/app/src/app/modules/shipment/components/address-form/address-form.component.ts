...
import { ICountry, IAddress, UnknownObjectType } from '../../../../interfaces';
...
export class AddressFormComponent implements OnInit, OnDestroy {
  ...
  get countriesOpts(): ICountryOpt[] {
    return this.countries
      .filter((country) => DISCOVERY_BOX_VALID_COUNTRIES.includes(country.iso))
      .map((elem) => ({
        iso: elem.iso,
  ...
  private onZipInputValueChanges(value: string): void {
    this.isInvalidZip = this.checkDataSrv.isInvalidZip(this.countriesByIso, this.addressForm.get('country')?.value, value);
  }
  ...
  private setInnerLoader(set: boolean, page: boolean): void {
    this.eventSrv.dispatchEvent('loading-animation', { set, isPage: page });
  }

  private logError(err: UnknownObjectType): void {
    this.loggerSrv.error('address-form-error', err);
  }
}
